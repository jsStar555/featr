...
class ChatsViewModel: ObservableObject {
    ...
    @MainActor
        func sendMessage(_ messageText: String) async  {
            guard let service = service else {
                fatalError("The service was not initialized before sending a message")
            }
            do {
                sendableError = false
                showAlert = false
    ...
    @MainActor
        func sendMessages(_ messageText: String) async throws {
            ...
            guard let service = service else {
                fatalError("The service was not initialized before sending messages")
            }
            if imageItems.count > 0 {
                var images: [UIImage] = []
                for image in imageItems {
        ...
        func updateMessageStatusIfNecessary() async throws {
            guard let service = service else {
                fatalError("The service was not initialized before updating the message status")
            }

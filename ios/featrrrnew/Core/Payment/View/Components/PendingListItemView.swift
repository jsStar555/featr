     ...
     func preparePaymentSheet() {
        ...
        if let job {
            Task {
                ...
                guard let fromUser = try? await UserService.fetchUser(withUid: jobProposal.buyerID) else {
                    paymentError = "Can't access the current buyers user's object"
                    showPaymentError = true
                    Log.d("Can't access the current seller user \(jobProposal.buyerID)")
                    return
                }
                PaymentService.standard.preparePaymentSheet(payment: calculateCost(), job: job, fromUser: fromUser, toUser: toUser) { [weak self] pS in
